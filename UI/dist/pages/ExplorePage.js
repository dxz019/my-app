...
const ExplorePage = ({ posts: propPosts, token, currentUser, showToast, fetchPosts, requireAuth, updatePostLikes }) => {
    ...
    const trendingPosts = useMemo(() => {
        ...
        if (activeTag) {
            ...
            filtered = filtered.filter(post => post.content.toLowerCase().includes(`#${tagLower}`) ||
    ...
    return (<div className="w-full" style={{ padding: '100px 2rem 3rem' }}>
            ...
            : activeQuery
                ...
                {(activeTag || activeQuery) && (<Button label="Clear Filter" icon="pi pi-times" className="p-button-text p-button-rounded p-button-plain text-primary" onClick={() => setSearchParams({})}/>)}
            </div>

            <div className="flex flex-column gap-4">
                <div className="grid">
                    {trendingPosts.map((post) => (<div key={post.id} className="col-12">
                            <PostCard key={post.id} post={post} currentUser={currentUser} token={token} comments={comments} showComments={showComments} onToggleComments={handleToggleComments} onAddComment={async (postId, text) => {
                if (!requireAuth())
                    return;
                try {
            ...
            }} onDeletePost={fetchPosts} onDeleteComment={async (postId) => {
                await fetchCommentsForPosts([postId], true);
                await fetchPosts();
            }} onUpdatePostLikes={updatePostLikes} requireAuth={requireAuth}/>
                        </div>))}
                </div>

                {trendingPosts.length === 0 && (<div className="surface-card p-8 border-round-2xl text-center border-1 surface-border shadow-1 mt-4">
