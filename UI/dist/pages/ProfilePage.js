...
const ProfilePage = ({ posts: propPosts, token, currentUser, showToast, fetchPosts, requireAuth, refreshCurrentUser }) => {
    ...
    const handleSaveProfile = async () => {
        ...
        const payload = Object.fromEntries(Object.entries(editForm).filter(([, value]) => typeof value === 'string' || value !== undefined));
        setLoading(true);
        try {
    ...
    const formatActivityTitle = (item) => {
        if (item.type === 'post')
            return `Shared a thought: ${item.title || 'Untitled'}`;
        if (item.type === 'comment')
            return `Commented on: ${item.title || 'a thought'}`;
        if (item.type === 'like')
            return `Liked a thought: ${item.title || 'Untitled'}`;
        return 'Recent activity';
    };
    if (loading && !displayUser) {
        return (<div className="flex justify-content-center p-8">
                <i className="pi pi-spin pi-spinner text-4xl text-primary"></i>
            </div>);
    }
    if (!displayUser && !loading) {
        return (<div className="surface-card p-6 border-round-xl text-center shadow-md">
    ...
    return (<div className="profile-wrapper w-full" style={{ padding: '0 2rem 3rem', marginTop: '20px' }}>
            {/* Header Section */}
            <div className="p-4 mb-4 mx-3" style={{
            ...
            boxShadow: 'var(--elevation-1)'
                ...
                <div className="flex flex-column md:flex-row align-items-center md:align-items-start gap-5">
                    ...
                    <div className="flex flex-column align-items-center gap-3">
                        <Avatar image={isEditing ? getPublicUrl(editForm.avatar_url) : getPublicUrl(displayUser.avatar_url)} label={!isEditing && !displayUser.avatar_url ? displayUser.username?.charAt(0).toUpperCase() : null} size="xlarge" shape="circle" style={{
            width: '110px',
            height: '110px',
            ...
            border: '4px solid var(--color-border)'
                        ...
                        {isEditing && (<div className="flex flex-column align-items-center gap-3">
                                <Button type="button" label={uploading ? "Uploading..." : "Change Avatar"} icon={`pi ${uploading ? 'pi-spin pi-spinner' : 'pi-camera'}`} className="p-button-outlined p-button-sm p-button-rounded" onClick={() => fileInputRef.current.click()} disabled={uploading}/>
                                <Button type="button" label="Create Avatar" icon="pi pi-palette" className="p-button-outlined p-button-sm p-button-rounded" onClick={() => setShowAvatarBuilder(true)}/>
                            </div>)}
                        <input type="file" ref={fileInputRef} style={{ display: 'none' }} accept="image/*" onChange={handleFileChange}/>
                    </div>

                    <div className="flex-1 text-center md:text-left pt-2">
                        {isEditing ? (<div className="flex flex-column gap-4 max-w-25rem mx-auto md:mx-0">
                                <div className="flex flex-column gap-2 mb-2">
                                    <label className="text-sm font-bold text-500">Full Name</label>
                                    <InputText value={editForm.full_name} onChange={(e) => setEditForm(prev => ({ ...prev, full_name: e.target.value }))} placeholder="Full Name" className="profile-edit-input" style={{
                ...
                color: 'var(--color-input-text)'
                                ...
                                <div className="flex flex-row gap-3">
                                    <div className="flex-1 flex flex-column gap-2">
                                        ...
                                        <InputText value={editForm.username} onChange={(e) => setEditForm(prev => ({ ...prev, username: e.target.value }))} placeholder="username" className="profile-edit-input" style={{
                background: 'var(--color-input-bg)',
                border: '1px solid var(--color-border)',
                borderRadius: '12px',
                color: 'var(--color-input-text)'
                                    ...
                                    <div className="flex-1 flex flex-column gap-2">
                                        ...
                                        <InputText value={editForm.email} onChange={(e) => setEditForm(prev => ({ ...prev, email: e.target.value }))} placeholder="email@example.com" className="profile-edit-input" style={{
                background: 'var(--color-input-bg)',
                border: '1px solid var(--color-border)',
                borderRadius: '12px',
                color: 'var(--color-input-text)'
                                ...
                                <div className="flex flex-column gap-2">
                                    ...
                                    <InputTextarea value={editForm.biography} onChange={(e) => setEditForm(prev => ({ ...prev, biography: e.target.value }))} placeholder="Tell the world about yourself..." rows={4} autoResize className="profile-edit-input" style={{
                background: 'var(--color-input-bg)',
                border: '1px solid var(--color-border)',
                borderRadius: '12px',
                color: 'var(--color-input-text)'
                                ...
                                <div className="flex align-items-center justify-content-center md:justify-content-start gap-4 mb-3">
                                    ...
                                    {isOwnProfile && currentUser && (<Button icon="pi pi-user-edit" className="p-button-rounded p-button-text p-button-lg" style={{ color: 'var(--color-primary)' }} onClick={() => {
                    setIsEditing(true);
                    setEditForm({
            ...
            <div className="p-4 mb-4" style={{ backgroundColor: 'var(--color-bg-elevated)', border: '2px solid var(--color-border)', borderRadius: '16px', boxShadow: 'var(--elevation-1)' }}>
                ...
                {activity.length > 0 ? (<div className="flex flex-column gap-3">
                        {activity.map((item) => (<div key={`${item.type}-${item.entity_id}`} className="p-3 border-round-xl" style={{ backgroundColor: 'var(--color-bg-card)', border: '1px solid var(--color-border)' }}>
                    ...
                    </div>) : (<p className="m-0" style={{ color: 'var(--color-text-muted)' }}>No activity yet.</p>)}
            </div>
            ...
            <div className="px-3">
                ...
                {userPosts.length > 0 ? (<div className="flex flex-column gap-4">
                        {userPosts.map((post) => (<div key={post.id} className="col-12">
                                <PostCard key={post.id} post={post} currentUser={currentUser} token={token} comments={comments} showComments={showComments} onToggleComments={(postId) => setShowComments(prev => ({ ...prev, [postId]: !prev[postId] }))} onAddComment={() => { }} onDeletePost={fetchPosts} onDeleteComment={async (postId) => {
                    await fetchCommentsForPosts([postId], true);
                    await fetchPosts();
                }} requireAuth={requireAuth}/>
                            </div>))}
                    </div>) : (<div className="text-center p-8 mt-6 surface-card border-round-2xl border-1 border-dashed" style={{ borderColor: 'var(--color-border)' }}>
                        ...
                        {isOwnProfile && !currentUser && (<Button label="Start Sharing" className="mt-5 p-button-text font-bold text-xl" style={{ color: 'var(--color-primary)' }} onClick={() => requireAuth()}/>)}
                    </div>)}
            </div>

            {/* Avatar Builder Dialog */}
            <Dialog header="Create Your Avatar" visible={showAvatarBuilder} style={{ width: '60vw', maxHeight: '85vh' }} modal draggable={false} onHide={() => setShowAvatarBuilder(false)} closeOnEscape dismissableMask contentStyle={{ maxHeight: '60vh', overflow: 'auto' }}>
                <AvatarBuilder onSave={(avatarUrl) => {
            if (avatarUrl) {
