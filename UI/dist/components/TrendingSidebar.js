...
const TrendingSidebar = () => {
    ...
    return (<div className="flex flex-column gap-4 sticky" style={{ top: '100px', maxWidth: '350px' }}>
            {/* Trending Section */}
            <div className="surface-card p-4 shadow-3 border-round-2xl border-1 surface-border">
                ...
                <div className="flex flex-column gap-2">
                    {trends.map((trend) => (<div key={trend.id} className="flex flex-column gap-1 p-3 border-round-xl cursor-pointer hover:bg-black-alpha-10 transition-all transition-duration-200" onClick={() => {
            ...
            <div className="surface-card p-4 shadow-3 border-round-2xl border-1 surface-border">
                ...
                <div className="flex flex-column gap-3">
                    {loading ? (<div className="flex justify-content-center p-4">
                            <i className="pi pi-spin pi-spinner text-primary"></i>
                        </div>) : (suggestedUsers.map((user) => (<div key={user.id} className="flex align-items-center justify-content-between gap-2 p-2 hover:bg-black-alpha-5 border-round-xl transition-all">
                                <div className="flex align-items-center gap-3 cursor-pointer overflow-hidden" onClick={() => navigate(`/profile/${user.id}`)}>
                                    <Avatar image={getPublicUrl(user.avatar_url)} label={!user.avatar_url ? user.username.charAt(0).toUpperCase() : null} shape="circle" style={{ backgroundColor: 'var(--color-primary)', color: '#fff', flexShrink: 0 }}/>
