...
const AvatarBuilder = ({ onSave, initialConfig = null }) => {
    ...
    return (<div className="avatar-builder p-4">
            <div className="preview flex flex-column align-items-center mb-4">
                <div className="avatar-preview-container p-3 mb-3" style={{
            background: 'var(--color-bg-elevated)',
            borderRadius: '50%',
            border: '4px solid var(--color-primary-light)',
            boxShadow: 'var(--shadow-lg)'
        }}>
                    {avatarUrl && (<img src={avatarUrl} alt="Avatar preview" style={{ width: 220, height: 220, borderRadius: '50%' }}/>)}
                </div>
                <Button label="Shuffle / Randomize" icon="pi pi-shuffle" className="p-button-rounded p-button-outlined font-bold" onClick={randomize}/>
            </div>

            <div className="controls grid mt-2">
                <div className="col-12 md:col-6 mb-3">
                    <label className="block text-sm font-bold text-500 mb-2">Avatar Style</label>
                    <Dropdown value={options.style} options={styles} onChange={(e) => updateOption('style', e.value)} className="w-full border-round-xl" placeholder="Select style"/>
                </div>
                
                {options.style === 'avataaars' && (<>
                        <div className="col-12 md:col-6 mb-3">
                            <label className="block text-sm font-bold text-500 mb-2">Eyes</label>
                            <Dropdown value={options.eyes} options={eyesOptions.map(v => ({ label: v, value: v }))} onChange={(e) => updateOption('eyes', e.value)} className="w-full border-round-xl" placeholder="Select eyes"/>
                        </div>
                        <div className="col-12 md:col-6 mb-3">
                            <label className="block text-sm font-bold text-500 mb-2">Mouth</label>
                            <Dropdown value={options.mouth} options={mouthOptions.map(v => ({ label: v, value: v }))} onChange={(e) => updateOption('mouth', e.value)} className="w-full border-round-xl" placeholder="Select mouth"/>
                        </div>
                    </>)}
