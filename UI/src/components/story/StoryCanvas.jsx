...
export default function StoryCanvas({ story, onSave, onNodeSelect }) {
  ...
  useEffect(() => {
    if (story?.nodes) setNodes(story.nodes)
    if (story?.edges) setEdges(story.edges)
  }, [story])
  ...
  const addNode = useCallback((type) => {
    ...
    requestAnimationFrame(() => {
      ...
      if (el) gsap.fromTo(el, { scale: 0.6, opacity: 0 }, { scale: 1, opacity: 1, duration: .35, ease: 'back.out(1.6)' })
    })
  }, [edges, triggerSave])
  ...
  return (
    <div className="story-canvas">
      ...
      >
        ...
        <MiniMap
          nodeColor={(n) => {
            if (n.type === 'video') return '#F5A623'
            if (n.type === 'choice') return '#7C6AF7'
            if (n.type === 'animation') return '#22D9A0'
            return '#2A2A38'
          }}
