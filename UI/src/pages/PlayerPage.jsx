...
export default function PlayerPage({ token }) {
  ...
  if (!started) {
    return (
      <div className="player-preroll flex flex-column align-items-center justify-content-center">
        <h1 className="mb-3">{currentStory.title}</h1>
        {currentStory.description && (
          <p className="mb-4 text-secondary">{currentStory.description}</p>
        )}
          ...
          <div className="flex flex-column gap-2">
            {(currentNode.data?.choices || []).map(choice => (
              <Button
                key={choice.id}
                label={choice.text}
                onClick={() => handleChoose(choice.targetNodeId)}
