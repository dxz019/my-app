...
vi.mock('primereact/inputtext', () => ({
    InputText: ({ value, onChange, placeholder, ...props }) => (
        <input
            value={value || ''}
            onChange={onChange}
...
vi.mock('primereact/password', () => ({
    Password: ({ value, onChange, placeholder, toggleMask, ...props }) => (
        <div style={{ width: '100%' }}>
            <input
                type={toggleMask ? 'password' : 'text'}
                value={value || ''}
                onChange={onChange}
                placeholder={placeholder}
                {...props}
...
vi.mock('primereact/inputtextarea', () => ({
    InputTextarea: ({ value, onChange, placeholder, ...props }) => (
        <textarea
            value={value || ''}
            onChange={onChange}
            placeholder={placeholder}
